...
st.set_page_config(
    page_title="Finance Trading Agent",
    page_icon="💰",
    layout="wide"
)
...
st.title("💰 Finance Trading Agent")
st.caption(f"Last updated: {uk_now.strftime('%d/%m/%Y %H:%M:%S')} (UK time)")
st.caption("⏱️ Auto refreshes every 30 minutes — Monitoring Legal & General")
st.progress(1.0)
...
def send_alert_email(alert_level, alert_msg, stock_name, current_price, todays_change, ai_analysis, gmail_address, gmail_password, alert_emails):
    try:
        ...
        body = f"""
Finance Trading Agent Alert — {stock_name}
...
st.subheader("💰 Legal & General (LGEN.L)")
